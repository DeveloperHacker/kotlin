...
fun showNotification(notification: Notification): String = when {
    notification is like Email(val email, val title, _) -> "You got an email from $email with title: $title"
    notification is like SMS(val number, val message) -> "You got an SMS from $number! Message: $message"
    notification is like VoiceRecording(val name, val link) -> "You received a Voice Recording from $name! Click the link to hear it: $link"
    else -> throw java.lang.IllegalStateException("Unexpected else")
}

fun showImportantNotification(notification: Notification, importantPeopleInfo: Sequence<String>): String = when {
    notification is like Email(val email, _, _) && importantPeopleInfo.contains(email) -> "You got an email from special someone!"
    notification is like SMS(val number, _) && importantPeopleInfo.contains(number) -> "You got an SMS from special someone!"
    notification is like val other -> showNotification(other)
}
