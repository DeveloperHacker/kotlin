...
class IntWrapper(val value: Int) {
    ...
    override fun equals(other: Any?) = when {
        other is Equlitive -> other == value
        other is like IntWrapper(val o) -> value == o
        else -> value == other
    }
}
...
class Equlitive(val value: IntWrapper) {
    ...
    override fun equals(other: Any?) = when {
        other is like Equlitive(val o) -> o == value
        other is like IntWrapper(val o) -> o == value + 5
        other is like val o is Int -> o == value + 5
        else -> false
    }
}

fun foo(a: Equlitive, eq: Int) = when {
    a is like (eq) -> 1
    a is like (eq eq) -> 2
    a is like eq eq -> 3
    a !is like (eq) -> 4
...
fun foo2(a: Equlitive, eq: Int) = when {
    a is like (eq eq) -> 2
    a is like (eq) -> 1
    a is like eq eq -> 3
    a !is like (eq eq) -> 5
...
fun foo3(a: Equlitive, eq: Int, _eq: Equlitive) = when {
    a is like (eq) -> 1
    a is like (_eq) -> 2
    a is like (_eq eq eq) -> 5
    a is like (_eq eq _eq) -> 6

    a is like (eq eq) -> 3
    a is like (eq _eq) -> 4
    a is like (eq _eq eq eq) -> 7
    a is like (eq _eq eq _eq) -> 8

    a is like eq eq -> 13
    a is like eq _eq -> 14
    a is like eq _eq eq eq -> 17
    a is like eq _eq eq _eq -> 18
...
fun foo4(a: Equlitive, eq: Int, _eq: Equlitive) = when {
    a is like (eq eq) -> 3
    a is like (eq _eq) -> 4
    a is like (eq _eq eq eq) -> 7
    a is like (eq _eq eq _eq) -> 8
...
fun foo5(a: Equlitive, eq: Int, _eq: Equlitive) = when {
    a is like eq eq -> 13
    a is like eq _eq -> 14
    a is like eq _eq eq eq -> 17
    a is like eq _eq eq _eq -> 18
