...
fun showImportantNotification(notification: Notification, importantPeopleInfo: Sequence<String>): String = when (notification) {
    is like Email(val email, _, _) && importantPeopleInfo.contains(email) -> "You got an email from special someone!"
    is like SMS(val number, _) && importantPeopleInfo.contains(number) -> "You got an SMS from special someone!"
    is like val other -> showNotification(other)
}
