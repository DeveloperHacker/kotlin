...
fun foo(x: Any?, a: Int) = when (<!DEBUG_INFO_SMARTCAST!>x<!>) {
    ...
    is like <!DUPLICATE_LABEL_IN_WHEN!>Pair<!>(10, 20) -> a + <!UNRESOLVED_REFERENCE!>b<!>
    is <!NO_TYPE_ARGUMENTS_ON_RHS, DUPLICATE_LABEL_IN_WHEN!>Pair<!> -> 1
    is like <!DUPLICATE_LABEL_IN_WHEN!>Pair<!>(_, val <!NAME_SHADOWING, UNUSED_VARIABLE!>a<!> is Pair<*, *>) -> 1
    is C<*, *> -> 2
