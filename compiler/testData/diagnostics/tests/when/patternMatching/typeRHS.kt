...
fun foo1(x: Any?) = when (<!DEBUG_INFO_SMARTCAST!>x<!>) {
...
fun foo2(x: Pair<Pair<Int, Int>, Any?>) = when (x) {
    ...
    <!USELESS_IS_CHECK!>is like <!USELESS_TYPE_CHECK, DUPLICATE_LABEL_IN_WHEN!>Pair<*, *><!>()<!> -> 0
    is like <!CANNOT_CHECK_FOR_ERASED, DUPLICATE_LABEL_IN_WHEN!>Pair<Int, Int><!>() -> 0
    is like <!USELESS_TYPE_CHECK, DUPLICATE_LABEL_IN_WHEN!>Pair<!>(val <!UNUSED_VARIABLE!>a<!> is <!USELESS_TYPE_CHECK!>Pair<!>) -> 0
    is like <!USELESS_TYPE_CHECK, DUPLICATE_LABEL_IN_WHEN!>Pair<!>(val <!UNUSED_VARIABLE!>a<!> is <!USELESS_TYPE_CHECK!>Pair<*, *><!>) -> 0
    is like <!USELESS_TYPE_CHECK, DUPLICATE_LABEL_IN_WHEN!>Pair<!>(val <!UNUSED_VARIABLE!>a<!> is <!USELESS_TYPE_CHECK!>Pair<Int, Int><!>) -> 0
    is like <!USELESS_TYPE_CHECK, DUPLICATE_LABEL_IN_WHEN!>Pair<!>(val <!UNUSED_VARIABLE!>a<!> = <!USELESS_TYPE_CHECK!>Pair<!>()) -> 0
    is like <!USELESS_TYPE_CHECK, DUPLICATE_LABEL_IN_WHEN!>Pair<!>(val <!UNUSED_VARIABLE!>a<!> = <!USELESS_TYPE_CHECK!>Pair<*, *><!>()) -> 0
    is like <!USELESS_TYPE_CHECK, DUPLICATE_LABEL_IN_WHEN!>Pair<!>(val <!UNUSED_VARIABLE!>a<!> = <!USELESS_TYPE_CHECK!>Pair<Int, Int><!>()) -> 0
    else -> 0
}